 ...
 */
//myclickdown 、myclickup 、 myclickok 、 mylongclick 、 unbind、 trigger   document触发然后在从 e.target向html节点搜索id/class，找到一个后终止
// $$为引用的变量名
//  可以是  class  id   obj  jqmobi_obj
//  注意：  class和id  相当与  live绑定 ， 不支持find元素后在绑定，这种使用obj绑定
//touche事件  $$("#/."+id).myclickdown(function(){$(this).dosomething   })
//           $$($("#"+id).find()).myclickok(function(){})
//           $$(obj).myclickok(function(){})

//需要挂载 bens  jq.mobi  依赖 device
            ...
            var handlerthis = function(obj){
                ...
                if(_this.savefn[_eventid]){
                    ...
                    if(_this.savefn[_eventid][type]){
                        _this.savefn[_eventid][type].call(obj);
                    }
                }
